...
import { upsertBike, markBikeSold, type BikeFormInput } from "@/lib/admin/actions";
import type { BikeRow } from "@/lib/admin/queries";
...
export function BikeForm({ bike }: { bike?: BikeRow }) {
  ...
  const [price, setPrice] = useState(bike?.price ?? 0);
  const [year, setYear] = useState(bike?.year ?? new Date().getFullYear());
  const [km, setKm] = useState(bike?.km ?? 0);
  const [type, setType] = useState<BikeType>(bike?.type ?? "hardtail");
  ...
  async function handleFiles(files: FileList | null) {
    ...
    for (const file of toUpload) {
      ...
      const { uploadBikePhoto } = await import("@/lib/admin/actions");
      const result = await uploadBikePhoto(formData);
      if ("url" in result) {
  ...
  function buildInput(): BikeFormInput {
    return {
      id: bike?.id,
      brand,
      model,
  ...
  function handleSave() {
    startTransition(() => upsertBike(buildInput()));
  }

  const isSelfListedLive = bike && bike.listing_type === "self" && bike.status === "live";

  return (
    <div className="rounded-card border border-line bg-surface p-5">
      <div className="flex items-center justify-between">
        <h1 className="text-h5 font-semibold">
          {bike ? `Edit · ${bike.brand} ${bike.model}` : "Add bike"}
        </h1>
      </div>
      ...
      <div className="mt-5 grid grid-cols-2 gap-4">
        ...
        <Field label="Type">
          <Select
            value={type}
            onValueChange={(next) => next && setType(next as BikeType)}
          >
      ...
      <div className="mt-6 flex items-center justify-between border-t border-line pt-4">
        ...
        {bike && (
          <DeleteBikeButton bikeId={bike.id} label={`${bike.brand} ${bike.model}`} variant="link" />
        )}
      </div>
    </div>
  );
